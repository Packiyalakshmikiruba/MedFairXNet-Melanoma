...
TRAIN_CSV = BASE_DIR / "data" / "processed" / "splits" / "train.csv"
VAL_CSV = BASE_DIR / "data" / "processed" / "splits" / "validation.csv"

MODEL_DIR = BASE_DIR / "models"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

MODEL_PATH = MODEL_DIR / "skin_lesion_efficientnet.keras"
...
print("=" * 70)
print("HAM10000 EFFICIENTNETB0 TRAINING")
print("=" * 70)
...
if train_df["label"].isna().any():
    raise ValueError("Unknown diagnosis found in training dataset.")

if val_df["label"].isna().any():
    raise ValueError("Unknown diagnosis found in validation dataset.")
...
missing_train = train_df[
    ~train_df["image_path"].apply(lambda x: Path(x).exists())
]

missing_val = val_df[
    ~val_df["image_path"].apply(lambda x: Path(x).exists())
]
...
if len(missing_val) > 0:
    raise FileNotFoundError("Some validation images are missing.")
